                              ...
                              float *kernel2, int lenInput, int myRank, float **output, int *lenOutput)
{
    ...
    (*output) = malloc( lenOut2 - (N - r) * sizeof *(*output));
    ...
    {   
        ...
        for ( i = 0; i < lenOut2; i++ )
        {
            if ( i >= lenOut2-(N - r) ) break;  // error here
            (*output)[i] = out2[i];
            (*lenOutput)++;
        }
                                  ...
                                  float *kernel2, float **output)
{
    ...
    else
    {
        ...
        int numEntries = lenOutput/NUM_OF_RANKS;                // 6
        int numBlocks_remainder = lenOutput%NUM_OF_RANKS;       // 0
        int numBlocks = NUM_OF_RANKS - numBlocks_remainder;     // 1
